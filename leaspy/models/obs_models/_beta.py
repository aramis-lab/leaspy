...
class FullBetaObservationModel(BetaObservationModel):
    ...
    def scalar_noise_std_update(
        ...
        *,
        ...
        noise_std: WeightedTensor[float],
    ) -> torch.Tensor:
        ...
        variance = torch.tensor([noise_std**2], requires_grad=True)
        ...
        for i in range(num_iterations):
            ...
            variance_dist = Beta(model * variance_pos, (1 - model) * variance_pos)

            # Compute the negative log-likelihood of the data under this beta distribution
            nll = -variance_dist.log_prob(y).mean()
            ...
            if (i + 1) % 100 == 0:
                print(f"Iteration {i + 1}: beta = {variance_pos.item():.4f}, NLL = {nll.item():.4f}")

        # Final optimized parameters
        variance_optimized = torch.nn.functional.softplus(variance).item()
        print(f"Optimized variance: {variance_optimized:.4f}")

        return torch.sqrt(variance_optimized)
