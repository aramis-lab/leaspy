...
class LMEPersonalizeAlgorithmTest(LeaspyTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.parameters = {
            "ages_mean": 0.0,
            "ages_std": 1.0,
            "fe_params": np.array([0.3333016, 1.0]),
            "cov_re": np.array([[0.4523892]]),
            "cov_re_unscaled_inv": np.array([[1 / 1.41324825e10]]),
        ...
        cls.values = np.array([[2.0], [4.0], [float("nan")], [8.0]])
    ...
    def test_remove_nans(self):
        ...
        self.assertTrue((new_values == np.array([2.0, 4.0, 8.0])).all())
        self.assertTrue((new_times.numpy() == np.array([0, 2, 6])).all())

    def test_get_individual_random_effect(self):
        ind_ip, res = self.algo._get_individual_random_effects_and_residuals(
    ...
    def test_run(self):
        ...
        self.assertAlmostEqual(
            ip._individual_parameters["pat1"]["random_intercept"], 3, 0
        )
        self.assertAlmostEqual(
            ip._individual_parameters["pat2"]["random_intercept"], 15, 0
        )
        self.assertAlmostEqual(
            ip._individual_parameters["pat3"]["random_intercept"], 6, 0
        )
