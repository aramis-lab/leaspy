...
class LMEFitAlgorithmTest(LeaspyTestCase):
    ...
    def setUpClass(cls):
        ...
        data = Data.from_dataframe(
            df, drop_full_nan=False
        )  # otherwise first visit of pat1 is dropped and then order of subjects is not as originally expected.........
    ...
    def test_get_reformated(self):
        ...
        expected_ages = np.array(
            self.dataframe.sort_index(axis=0).index.get_level_values("TIME")
        )[1:]
        values = self.algo._get_reformated(self.dataset, "values")
        expected_values = np.array(self.dataframe.sort_index(axis=0)["feat1"].values)[
            1:
        ]
    ...
    def test_run(self):
        ...
        self.assertAlmostEqual(
            self.model.parameters["fe_params"][0], 8 + ages_mean * 1.0, 0
        )
        self.assertAlmostEqual(self.model.parameters["fe_params"][1], 1.0 * ages_std, 0)
        self.assertAlmostEqual(
            self.model.parameters["noise_std"], 1.3220877080541017e-05
        )
        self.assertAlmostEqual(self.model.parameters["cov_re"][0][0], 2.8888, 3)
        self.assertAlmostEqual(
            self.model.parameters["cov_re_unscaled_inv"][0][0], 1 / 14e9, -9
        )
        self.assertAlmostEqual(self.model.parameters["bse_fe"][0], 1, 0)
        self.assertAlmostEqual(self.model.parameters["bse_fe"][1], 0, 0)
        self.assertAlmostEqual(
            self.model.parameters["bse_re"][0], 61806.329294854615, -3
        )
