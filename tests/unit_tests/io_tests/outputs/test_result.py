...
class ResultTest(LeaspyTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.cofactors = pd.read_csv(
            cls.example_data_covars_path, dtype={"ID": str}
        ).set_index("ID")
        cls.data.load_cofactors(cls.cofactors)
        cls.df = cls.data.to_dataframe()
        load_individual_parameters_path = cls.from_personalize_ip_path(
    ...
    def test_constructor(self):
        ...
        self.assertEqual(
            list(self.results.individual_parameters.keys()), ["tau", "xi", "sources"]
        )
    ...
    def _save_and_compare(
        self, individual_parameter_filename: str, method: Method, *args, **kwargs
    ):
        path_to_expected_individual_parameters = self.from_personalize_ip_path(
            individual_parameter_filename
        )
        path_to_computed_individual_parameters = self.get_test_tmp_path(
            individual_parameter_filename
        )
        self._saving_method_factory(method)(
            path_to_computed_individual_parameters, *args, **kwargs
        )
        self.assertDictAlmostEqual(
            self._loading_method_factory(method)(
                path_to_expected_individual_parameters
            ),
            self._loading_method_factory(method)(
                path_to_computed_individual_parameters
            ),
        )
    ...
    def test_save_individual_parameters_default_json(self):
        self._save_and_compare(
            "data_tiny-individual_parameters.json", Method.JSON, indent=None
        )
    ...
    def test_save_individual_parameters_subset_of_subjects_csv(self):
        self._save_and_compare(
            "data_tiny-individual_parameters-3subjects.csv", Method.CSV, self.idx_sub
        )
    ...
    def test_load_individual_parameters(self):
        self.generic_check_individual_parameters(
            self.results.individual_parameters, nb_individuals=17
        )

    def test_load_result(self):
        individual_parameter_input_list = [
            self.from_personalize_ip_path(
                f"data_tiny-individual_parameters.{extension}"
            )
        ...
        for data_input in (self.data, self.df, self.example_data_path):
            for ind_param_input in individual_parameter_input_list:
                with self.subTest(ip_path=ind_param_input, data=data_input):
                    self._load_result_and_check_same_as_expected(
                        ind_param_input, data_input
                    )

    def _load_result_and_check_same_as_expected(self, ind_param, data):
        results = Result.load_result(
            data, ind_param, cofactors=self.example_data_covars_path
        )
        new_df = results.data.to_dataframe()
        pd.testing.assert_frame_equal(new_df, self.df)
        self.generic_check_individual_parameters(
            ind_param=results.individual_parameters, nb_individuals=17
        )
    ...
    def test_get_cofactor_distribution(self):
        self.assertEqual(
            self.results.get_cofactor_distribution("Treatments"),
            self.cofactors.values.reshape(self.cofactors.shape[0]).tolist(),
        )

    def test_get_patient_individual_parameters(self):
        self.assertAlmostEqual(
            self.results.get_patient_individual_parameters("116")["tau"].tolist()[0],
            79.124,
            delta=1e-3,
        )
        self.assertAlmostEqual(
            self.results.get_patient_individual_parameters("116")["xi"].tolist()[0],
            0.5355,
            delta=1e-3,
        )
        self.assertAlmostEqual(
            self.results.get_patient_individual_parameters("116")["sources"].tolist()[
                0
            ],
            3.7742,
            delta=1e-3,
    ...
    def test_get_parameter_distribution(self):
        self.assertEqual(
            self.results.get_parameter_distribution("xi"),
            self.results.individual_parameters["xi"].view(-1).tolist(),
        )
        self.assertEqual(
            self.results.get_parameter_distribution("tau"),
            self.results.individual_parameters["tau"].view(-1).tolist(),
        )
        self.assertEqual(
            self.results.get_parameter_distribution("sources"),
            {
                "sources" + str(i): val
                for i, val in enumerate(
                    self.results.individual_parameters["sources"]
                    .transpose(1, 0)
                    .tolist()
                )
            },
        ...
        self.assertEqual(
            list(xi_treatment_param.keys()), ["Treatment_A", "Treatment_B"]
        )
        self.assertEqual(
            len(xi_treatment_param["Treatment_A"]),
