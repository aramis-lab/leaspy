...
class AttributesLinearTest(LeaspyTestCase):
    def test_constructor(self):
        ...
        self.assertRaises(
            ValueError, LinearAttributes, "name", "4", 3.2
    ...
    def test_compute_orthonormal_basis(self):
        ...
        names = {"all"}
        values = {
            "g": torch.tensor([0.0] * 4),
            "v0": torch.tensor(
                [-3.0, 1, 0, -1]
            ),  # as for logistic (too high v0values [exp'd] implies a precision a bit coarser)
            "betas": torch.tensor(
        ...
        for i in range(dimension - 1):
            ...
            self.assertAlmostEqual(
                torch.dot(orthonormal_vector, dgamma_t0).item(), 0, delta=1e-6
            )
    ...
    def test_mixing_matrix_utils(self):
        ...
        for mixing_column in mixing_matrix.permute(1, 0):
            # Test orthogonality to dgamma_t0 (canonical inner-product)
            self.assertAlmostEqual(
                torch.dot(mixing_column, dgamma_t0).item(), 0, delta=1e-6
            )
