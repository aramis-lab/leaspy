...
from leaspy.models.utils.attributes.logistic_parallel_attributes import (
...
class AttributesLogisticParallelTest(LeaspyTestCase):
    def test_constructor(self):
        ...
        for attr_name in ("positions", "velocities", "mixing_matrix"):
            self.assertTrue(
                isinstance(getattr(attributes, attr_name), torch.FloatTensor)
            )
        ...
        self.assertEqual(
            attributes.update_possibilities, {"all", "g", "betas", "deltas"}
        )
        self.assertRaises(
            ValueError, LogisticParallelAttributes, "name", "4", 3.2
        )  # with bad type arguments
        self.assertRaises(TypeError, LogisticParallelAttributes)  # without argument

        self.assertRaises(
            ValueError, attributes._check_names, {"xi_mean"}
        )  # was USELESS so removed
        self.assertRaises(
            ValueError, attributes._check_names, {"v0"}
        )  # only for multivariate not parallel
        self.assertRaises(
            ValueError, attributes._check_names, {"v0_collinear"}
        )  # only for multivariate not parallel
    ...
    def compute_instance_and_variables(self):
        ...
        values = {
            "g": torch.tensor([0.0]),
            "deltas": torch.tensor([-1.0, 0.0, 2.0]),
            "betas": torch.tensor(
        ...
        attributes = LogisticParallelAttributes(
            "logistic_parallel", dimension, source_dimension
        )
        attributes.update(names, values)

        # Test the first value of the derivative of gamma at t0
        p0 = 1.0 / (1.0 + torch.exp(values["g"]))
        collin_standard_v0 = (
            p0 * (1 - p0)
        )  # do not multiply by scalar constants... esp. the removed torch.exp(values['xi_mean'])
        ...
        return (
            attributes,
            collin_dgamma_t0,
            sqrt_metric_normalization,
            collin_standard_v0,
        )

    def test_compute_orthonormal_basis(self):
        attributes, collin_dgamma_t0, sqrt_metric_norm, collin_standard_v0 = (
            self.compute_instance_and_variables()
        )
        ...
        for i in range(attributes.dimension - 1):
            ...
            self.assertAlmostEqual(
                torch.dot(
                    orthonormal_vector, collin_dgamma_t0 / sqrt_metric_norm**2
                ).item(),
                0,
                delta=1e-6,
            )  # / sqrt_metric_norm
            ...
            self.assertAlmostEqual(
                torch.norm(orthonormal_vector).item(), 1, delta=1e-6
            )  # /sqrt_metric_norm
            # Test orthogonality to other vectors (canonical inner-product)
            for j in range(i + 1, attributes.dimension - 1):
                self.assertAlmostEqual(
                    torch.dot(orthonormal_vector, orthonormal_basis[:, j]).item(),
                    0,
                    delta=1e-6,
                )  # / sqrt_metric_norm

    def test_mixing_matrix_utils(self):
        attributes, dgamma_t0, sqrt_metric_norm, _ = (
            self.compute_instance_and_variables()
        )
        ...
        for mixing_column in mixing_matrix.permute(1, 0):
            # Test orthogonality to dgamma_t0 (metric inner-product)
            self.assertAlmostEqual(
                torch.dot(mixing_column, dgamma_t0 / sqrt_metric_norm**2).item(),
                0,
                delta=1e-6,
            )
