...
class SimulationAlgorithm(AbstractAlgo):
    ...
    def __init__(self, settings):
        ...
        self.sources_method: SourcesMethod = SourcesMethod(
            settings.parameters["sources_method"]
        )
        ...
        self.reparametrized_age_bounds = settings.parameters[
            "reparametrized_age_bounds"
        ]
        self.features_bounds = settings.parameters["features_bounds"]
        self.features_bounds_nb_subjects_factor: float = settings.parameters[
            "features_bounds_nb_subjects_factor"
        ]
    ...
    def _validate_number_of_subjects_and_visits(self):
        ...
        if self.min_number_of_visits < 1:
            raise LeaspyAlgoInputError(
                'The "min_number_of_visits" should be an integer >= 1'
            )
    ...
    def _validate_algo_parameters(self):
        ...
        self._validate_parameter_has_type(
            "features_bounds", (bool, dict), "a bool or a dictionary"
        )
        self._validate_parameter_has_type(
    ...
    def _check_cofactors(self, data: Data):
        ...
        if (
            len(unknown_cofactors := [f for f in self.cofactor if f not in cofactors])
            > 0
        ):
            raise LeaspyAlgoInputError(
        ...
        invalid_cofactors = dict(
            [
                ...
                for cofactor_name, cofactor_value in zip(
                    self.cofactor, self.cofactor_state
                )
    ...
    def _get_features_bounds(self, data: Data) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        return torch.tensor(df_scores.iloc[:, 1:].min()), torch.tensor(
            df_scores.iloc[:, 1:].max()
        )
    ...
    def _get_noise_distribution(
        ...
        if isinstance(self.noise, str):
            return self._get_noise_model_from_string(
                model, dataset, individual_parameters
            )
    ...
    def _get_noise_model_from_string(
        ...
        new_noise_model.update_parameters_from_predictions(dataset, predictions)
        return new_noise_model
    ...
    def _get_noise_model_from_numeric(self, model: AbstractModel) -> BaseNoiseModel:
        ...
        return GaussianDiagonalNoiseModel(
            {"scale": noise_scale}, scale_dimension=model.dimension
        )
    ...
    def _simulate_individual_parameters(
        ...
        *,
        ...
        samples = scaler.inverse_transform(
            samples
        )  # A np.ndarray of shape (n_subjects, n_features)
    ...
    def _subselect_individuals_in_features_bounds(
        ...
        subjects.timepoints = [
            v
            for i, v in enumerate(subjects.timepoints)
            if i in indices_of_accepted_subjects
        ]
        subjects.values = [
            v
            for i, v in enumerate(subjects.values)
            if i in indices_of_accepted_subjects
        ]
    ...
    def run_impl(
        ...
        """
        get_sources = "univariate" not in model.name and getattr(
        ...
        df_individual_parameters = results.get_dataframe_individual_parameters(
            cofactors=self.cofactor
        )
        if self.cofactor_state:
            for cofactor_name, cofactor_state in zip(
                self.cofactor, self.cofactor_state
            ):
        ...
        df_individual_parameters = (
            data.to_dataframe()
            .groupby("ID")
            .first()[["TIME"]]
            .join(df_individual_parameters, how="right")
        )
        ...
        if self.reparametrized_age_bounds:
            distribution = np.array(
                [
                    ind
                    for ind in distribution
                    if min(self.reparametrized_age_bounds)
                    < ind[0]
                    < max(self.reparametrized_age_bounds)
                ]
            )
        ...
        if self.features_bounds:
            self._subselect_individuals_in_features_bounds(
                simulated_subjects, data, n_target
            )
        ...
        indices = [
            self.prefix + str(i).rjust(len_subj_id, "0") for i in range(1, n_target + 1)
        ]
...
def _check_parameter_has_type(
    label: str, value, type_or_klass, type_desc: str, *, optional: bool = False
):
...
def _check_mean_min_max_order(
    parameter_name: str, minimum: float, mean: float, maximum: Optional[float] = None
):
...
def _get_mean_and_covariance_matrix(
    m: torch.Tensor,
...
def _get_subjects_in_features_bounds(
    ...
    return [
        i
        for i, baseline_score in enumerate(baseline_scores)
...
def _test_subject(
    ...
    return ((features_min <= baseline_scores) & (baseline_scores <= features_max)).all()
...
def _sample_sources(
    ...
    sigma_22 = (
        df_cov.narrow(0, 3, source_dimension).narrow(1, 3, source_dimension).clone()
    )
