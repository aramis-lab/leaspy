...
class CollectionRealization(DictRealizations):
    ...
    def __setitem__(self, variable_name: str, v) -> NoReturn:
        raise NotImplementedError(
            "Please use the setter from `.population` or `.individual` sub-collections."
        )
    ...
    def _check_unicity_of_variable_names(self):
        variable_names_intersection = set(self.population.names).intersection(
            set(self.individual.names)
        )
    ...
    def initialize(
        ...
        **realization_init_kws : dict
        ...
        self.initialize_population(
            model, skip_variable=skip_variable, **realization_init_kws
        )
