...
def realization_factory(
    realization_or_variable_type: RealizationFactoryInput, **kws
) -> AbstractRealization:
    """
    ...
    **kws
    ...
    if isinstance(realization_or_variable_type, str):
        realization_or_variable_type = VariableType.from_string(
            realization_or_variable_type
        )
    return _realization_as_variable_type_factory(realization_or_variable_type, **kws)


def _realization_as_variable_type_factory(
    variable_type: VariableType, **kws
) -> AbstractRealization:
